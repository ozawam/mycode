...
//    double total_disk_mass = 15.0*M_earth/M_sun;
    int N_planetesimals = 300;
    double rho_planet = _rho_planet * 1.49597871e13 * 1.49597871e13 * 1.49597871e13 / 1.9884e33; // [Msun/AU^3]
    double planetesimal_mass = 1.0e+24/M_sun;//total_disk_mass/N_planetesimals;//M_sun //unit:Msun;
...
//   r->softening     = 1.0*rsoft;        // Gravitational softening length
    ...
    double h = rH/acenter ;
    double sigma_e = 2.0*h  ; 
    double sigma_i = h  ; 
//    double sigma_e = sqrt(2.0*h/(2.0-(M_PI/2.0)))  ; 
//    double sigma_i = sqrt(h/(2.0-(M_PI/2.0)))  ; 
...
//        double inc  = reb_random_rayleigh(0.000001);       
        double e    = reb_random_rayleigh(sigma_e);
        double inc  = reb_random_rayleigh(sigma_i);
