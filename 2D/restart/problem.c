...
//Setup constants.
    r->dt                   = 0.001*2.0*M_PI;                // initial timestep
...
//########################################################
	...
	double rau;
	rau = sqrt( particles[i].x * particles[i].x + particles[i].y * particles[i].y + particles[i].z * particles[i].z ) ;

	if(rau <3.0){
	printf("Try to remove index i (i+1th particle)  while preserving the order with keepSorted=1...\n");
	...
	double rj;
	rj = sqrt( pow(particles[i].x - particles[1].x,2.0) + pow(particles[i].y - particles[1].y,2.0) + pow(particles[i].z - particles[1].z,2.0) ) ;

	if(rj <0.4){// Hill sphere of Jupiter ~ 0.354 AU 
	printf("Try to remove index i (i+1th particle)  while preserving the order with keepSorted=1...\n");
	success = reb_remove(r, i, keepSorted);
	if (success){
	printf("Particle successfully removed\n");
